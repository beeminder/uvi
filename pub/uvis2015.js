...
"c": "[twttr#1430] A legit check would fail if trying to BCC support &amp; \"highest_paid\" was not yet set. [happened a total of 6 times, and corrected w/in the day for all cases]",
}, /*************************************************************************/ {
...
"c": "[twttr#1440] Smart-submit a la https://twitter.com/beemuvi/status/344335764101926912",
}, /*************************************************************************/ {
...
"c": "[twttr#1445] Featured gallery has twice as many goals shown and sorts by most recently added data instead of most recent save",
}, /*************************************************************************/ {
...
"c": "[twttr#1448] ...before you'd actually filled out the form",
}, /*************************************************************************/ {
...
"c": "[twttr#1452] Three things with google auth: tell rails to remember the user before signing them in, update token info on login if it differs from current info (old-style Google auth?), programmatically revoke tokens when removing an authorization",
}, /*************************************************************************/ {
"f": true,
...
"c": "[twttr#1455] Stop saying the copyright year since that keeps getting out of date",
}, /*************************************************************************/ {
...
"c": "[twttr#1480] \"P.S. Pro tip: drop your pledge back down with the arrows above your graph.\"",
}, /*************************************************************************/ {
"f": true,
...
"c": "[twttr#1495] Still need general fix for \"redirects omg\"",
}, /*************************************************************************/ {
...
"c": "[twttr#1513] Does not happen every time, so I'm not sure exactly why this sometimes happened, but I just added the explicit date check to the \"is this goal recommitable?\" condition",
}, /*************************************************************************/ {
...
"c": "[twttr#1526] Then there was yet another UVI we're not counting where we made it actually count up all the threads instead of using gmail's estimate",
}, /*************************************************************************/ {
...
"c": "[twttr#1533] Robin Ryder (@Pruneau) suggested: \"Thank you for updating your card. The following charges are outstanding and will now be retried.\"",
}, /*************************************************************************/ {
"f": true,
...
"c": "[twttr#1534] Prefacing features with FEATURE and bugfixes with BUGFIX might be a good idea if we did it consistently...",
}, /*************************************************************************/ {
...
"c": "[twttr#1548] Use tini/vini for do less goals' initial safety buffer. Old way caused the road width to be 2x \"true\" width for the first day, so your Hard Limit numbers were kind of a lie. we'd been using a road row to jump the road because it was implemented before tini vini.",
}, /*************************************************************************/ {
...
"c": "[twttr#1553] Sadness because there's still periods of extreme slowness for this to happen in the first place. There's probably a way to word this that sounds more positive.",
}, /*************************************************************************/ {
...
"c": "[twttr#1555] orig: handled missing goal for add datapoint Action; prev UVI: handled missing goal for add datapoint Trigger; new UVI: handle missing goal for beemergency safebuf Trigger",
}, /*************************************************************************/ {
...
"c": "[twttr#1558] https://www.dropbox.com/s/ttg756ot309ekev/Screenshot%202015-05-21%2010.21.20.png?dl=0 HT sean fellows (drtall) gusthecorgi.com",
}, /*************************************************************************/ {
"f": true,
...
"c": "[twttr#1561] When/why do you have to refresh on Beeminder? you shouldn't in theory. though i don't know *how* quickly they push data out. There might be a delay? like IFTTT 'automatically' syncs, but you still have to wait 16 minutes",
}, /*************************************************************************/ ]
...
"c": "[twttr#1562] API users controller is still one of the slowest transactions, but now the slow calls are ~600ms as opposed to ~3000ms",
}, /*************************************************************************/ {
...
"c": "[twttr#1566] These are scheduled based on \"Sort Threshold\" (see Terrifyingly Advanced settings) but we're deprecating that",
}, /*************************************************************************/ {
...
"c": "[twttr#1574] Case for \"yesterday\" in helpful table: this happens if table is out of date, e.g. just after the day rolls over and before the goal is refreshed.",
}, /*************************************************************************/ {
...
"c": "[twttr#1576] Don't error if no color (e.g. goal error or new goal). chipmanaged asked for the deltas in the title text in case you've already reached the delta. fixes next_color 500 error",
}, /*************************************************************************/ {
...
"c": "[twttr#1579] For Do Less and weight loss goals safebump is not really meaningful",
}, /*************************************************************************/ {
...
"c": "[twttr#1582] Lame because it was a stupid bug and totally my fault, but fixed a bug in do less hardcap section where it was displaying like a stop sign &amp; missing the targets",
}, /*************************************************************************/ {
...
"c": "[twttr#1584] http://forum.beeminder.com/t/secret-featured/886",
}, /*************************************************************************/ {
...
"c": "[twttr#1606] Ratelimit is to protect against infinite loops since we have both IFTTT triggers &amp; actions around adding data",
}, /*************************************************************************/ {
...
"c": "[twttr#1608] We were counting up all the issues closed since midnight, but if the maint check doesn't run until 00:30 and there are additional issues closed btwn 00:00 and 00:30 when maintenance runs, then we'd count them for yesterday, and again tomorrow when we count up issues] -->",
}, /*************************************************************************/ {
...
"c": "[twttr#1617] We were probably trying to be too clever? now just give the option",
}, /*************************************************************************/ {
...
"c": "[twttr#1618] E.g. in misfit landing page the header text on the sign up/in modal said \"Sign in using google to get started with your sleep as android goal.\"",
}, /*************************************************************************/ {
...
"c": "[twttr#1624] I was lazily throwing the entire message content into the db, which failed to serialize msgs w/ attachments. now we store just the relevant reply params",
}, /*************************************************************************/ {
...
"c": "[twttr#1629] For gmailzero and runkeeper goals we now retry the data fetch in a few minutes if we get a Timeout error from the external service.",
}, /*************************************************************************/ {
...
"c": "[twttr#1630] For autodata goals we were doing the fetch-new-data part of user-initiated refreshes on a lower priority queue. Now refresh and fetch on highest prio queue.",
}, /*************************************************************************/ {
...
"c": "[twttr#1631] Previously tweeted for rescuetime and jawbone",
}, /*************************************************************************/ {
...
"c": "[twttr#1634] \"...derail you while you're gone!\"",
}, /*************************************************************************/ {
...
"c": "[twttr#1640] Since changing something with Mixpanel",
}, /*************************************************************************/ {
...
"c": "[twttr#1641] Confusion about what exactly the bug was, because seemed like it should have been universal problem, but wasn't",
}, /*************************************************************************/ {
...
"c": "[twttr#1642] They send an event with type \"MULTI_SPORT\" that is the sum total of all the multiple events recorded as part of the multi-sport triathlon training extravaganza. since they also send along each of the different multiple sports as their own stand-alone entries, the multi-sport one should just be ignored",
}, /*************************************************************************/ {
...
"c": "[twttr#1643] Only for the subset of ppl who didn't have any active goals with sms reminders on, i think, so maybe pretty inconsequential",
}, /*************************************************************************/ {
...
"c": "[twttr#1644] We'd previously only had this come up when you clicked Cancel on a pending archive",
}, /*************************************************************************/ {
...
"c": "[twttr#1650] Occasional error for custom or weight loss restarts from success state. seemed to be unclear, however, given the rate that the error was occurring",
}, /*************************************************************************/ {
...
"c": "[twttr#1667] When after midnight for a nightowl deadline, or after the deadline and before midnight for an early bird goal",
}, /*************************************************************************/ {
...
"c": "[twttr#1682] Used to just do now + AKH, now do floor(now) + AKH, now the archive *def* comes before the derail email on the final day would kick in",
}, /*************************************************************************/ {
"n": 1683,
"f": true,
...
"c": "[twttr#1704] If we passed the deadline while running the job, or while it was queued, we'd send the Eep! alert anyway. Skip this too-late alert now.",
}, /*************************************************************************/ {
...
"c": "[twttr#1708] aka the 10:06am bug",
}, /*************************************************************************/ {
...
"c": "[twttr#1709] Fix in the autodata polling job: if queues were backed up, and more than an hour passed since the last job. spurious derails addressed.",
}, /*************************************************************************/ {
...
"c": "[twttr#1710] Fix 500 error from queue-for-refresh on new goals",
}, /*************************************************************************/ {
...
"c": "[twttr#1711] http://forum.beeminder.com/t/ifttt-and-manual-entry-fiasco/720/6?u=dreev",
}, /*************************************************************************/ {
...
"c": "[twttr#1712] If the response code is not 200, then the data attribute will be blank, so don't check for that until you've checked for 401 unauthorized or other error codes in the response",
}, /*************************************************************************/ {
...
"c": "[twttr#1713] Now we fetch data since the last non-reset datapoint",
}, /*************************************************************************/ {
...
"c": "[twttr#1772] \"Deadline cannot be moved into the past, since this goal is in the red currently, and that would derail you. Please finish your task for the day first.\"",
}, /*************************************************************************/ {
